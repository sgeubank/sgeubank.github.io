...
class axZmH
{
    ...
    public function checkConfig($zoom)
    {
        ...
        if (!defined('AJAXZOOM_PLUGIN_SKIP_LIC') || AJAXZOOM_PLUGIN_SKIP_LIC != true) {
    ...
    public function rawThumbLoad($zoom)
    {
        ...
        } else if (!$this->isValidFilename($_GET['previewPic'])) {
            if (isset($_GET['textError'])) {
                echo 'Not a valid filename! Name: '.$_GET['previewPic'];
            } else {
                $im = imagecreatetruecolor((int)$_GET['width'], (int)$_GET['height']);
                $background_color = imagecolorallocate($im, 210, 210, 210);
                imagefill($im, 0, 0, $background_color);
                $text_color = imagecolorallocate($im, 171, 0, 0);
                imagestring($im, 2, 5, 5, 'Not a valid filename!', $text_color);
                imagestring($im, 1, 5, 25, 'Name: '.$_GET['previewPic'], $text_color);
                header('Content-Type: image/jpeg');
                header("Pragma: public");
                header("Cache-Control: maxage=1");
...
';
    ...
    public function zoomServerPar($ret, $parExcl = false, $parExclPreg = false, $queryString = false)
    {
        ...
        $parExclDefault = array('zoomID', 'zoomFile', 'zoomLoadAjax', 'loadZoomAjaxSet', 'load360AjaxSet', 'dc', '_');
        if (is_array($parExcl)) {
...
jQuery.fn.axZm('.$jsObject.');
    ...
    public function drawZoomJsConf($zoom, $rn = false, $pack = true)
    {
        ...
        $js .= 'jQuery.axZm.gallerySlideSwipeBackCond1 = '.$this->ptj($zoom['config']['gallerySlideSwipeBackCond1']).'; ';
        $js .= 'jQuery.axZm.gallerySlideSwipeBackCond2 = '.$this->ptj($zoom['config']['gallerySlideSwipeBackCond2']).'; ';
